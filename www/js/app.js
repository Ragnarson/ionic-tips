...
// 2
angular.module('app')
.factory('Navigator', ["$state", "$ionicHistory", "$ionicViewSwitcher", function($state, $ionicHistory, $ionicViewSwitcher) {
  return {
...
// 3
angular.module('app')
.directive('defaultNavBackButton', ["$ionicHistory", "$state", "$stateParams", "$ionicConfig", "$ionicViewSwitcher", "$ionicPlatform", function ($ionicHistory, $state, $stateParams, $ionicConfig, $ionicViewSwitcher, $ionicPlatform) {
  return {
    link: link,
    restrict: 'EA'
  };
...
angular.module('app')
.controller("HelloCtrl", ["$scope", "$stateParams", "Navigator", function($scope, $stateParams, Navigator) {
...
.config(["$provide", "$httpProvider", function($provide, $httpProvider) {
  $provide.factory('timeoutHandler', ["$q", "$injector", function($q, $injector) {
    ...
    return {
      responseError: function(rejection) {
        if(isTimeout(rejection)) {
          $injector.invoke(["$ionicPopup", function($ionicPopup) {
            $ionicPopup.alert({
              title: 'Timeout',
